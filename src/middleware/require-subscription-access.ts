...
export async function requireWorkspaceSubscriptionAccess(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    if (!access.allowed) {
      res.status(403).json({ error: { code: "SUBSCRIPTION_ACCESS_REQUIRED", message: access.reason, status: access.status } });
      return;
    }
    next();
  } catch (error) {
