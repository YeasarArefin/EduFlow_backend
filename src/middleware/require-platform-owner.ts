...
export async function requirePlatformOwner(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    const [owner] = await db.select({ userId: platformOwners.userId }).from(platformOwners).where(eq(platformOwners.userId, userId)).limit(1);
    if (!owner) {
      res.status(403).json({ error: { code: "PLATFORM_OWNER_REQUIRED", message: "Platform Owner access is required." } });
      return;
    }
    next();
  } catch (error) {
