...
import { planFeatures, subscriptions, workspaceEntitlementOverrides } from "../database/schema/subscriptions";
...
export async function resolveWorkspaceEntitlements(workspaceId: string): Promise<EffectiveWorkspaceEntitlements> {
  const [currentSubscription] = await db
    ...
    .from(subscriptions)
    .where(and(eq(subscriptions.workspaceId, workspaceId), inArray(subscriptions.status, currentSubscriptionStatuses)))
    .limit(1);
  ...
  for (const override of overrides) {
    const current = entitlements[override.featureKey] ?? {
      enabled: false,
      limit: null
    };
