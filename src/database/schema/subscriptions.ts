...
export const paymentRequestStatus = pgEnum("payment_request_status", ["pending", "approved", "rejected"]);

export const paymentRequestPurpose = pgEnum("payment_request_purpose", ["subscription", "sms_credit"]);

export const paymentMethod = pgEnum("payment_method", ["cash", "bkash", "nagad", "rocket", "other"]);
...
export const workspaceEntitlementOverrides = pgTable(
  ...
  (table) => [
    ...
    check(
      "workspace_entitlement_overrides_limit_nonnegative_chk",
      sql`${table.limitOverride} is null or ${table.limitOverride} >= 0`
    )
  ]
);
...
export const paymentRequests = pgTable(
  ...
  (table) => [
    ...
    check(
      "payment_requests_subscription_plan_chk",
      sql`${table.purpose} <> 'subscription' or ${table.planId} is not null`
    ),
    check("payment_requests_amount_minor_nonnegative_chk", sql`${table.amountMinor} >= 0`),
    check(
      "payment_requests_reviewed_at_status_chk",
