...
async function query(text, values = []) { return pool.query(text, values); }
function response() {
...
describe("subscription entitlement guards", () => {
  beforeAll(async () => {
    await query("INSERT INTO workspaces (id, name, slug, status) VALUES ($1, $2, $3, 'active')", [workspaceId, "Guard Workspace", `guard-${Date.now()}`]);
    await query("INSERT INTO plans (id, name, slug, duration_days, trial_days) VALUES ($1, $2, $3, 30, 0)", [planId, "Guard Plan", `guard-plan-${Date.now()}`]);
    await query("INSERT INTO features (key, name) VALUES ($1, $2)", [featureKey, "Guard Feature"]);
    await query("INSERT INTO plan_features (plan_id, feature_key, enabled, limit_value) VALUES ($1, $2, true, 10)", [planId, featureKey]);
    await query("INSERT INTO subscriptions (workspace_id, plan_id, status, expires_at) VALUES ($1, $2, 'active', now() + interval '1 day')", [workspaceId, planId]);
  });
  afterAll(async () => {
  ...
  it("blocks a disallowed subscription with 403", async () => {
    ...
    const result = await run(requireWorkspaceSubscriptionAccess, { workspaceContext: { workspaceId } });
    expect(result).toMatchObject({ called: false, statusCode: 403, body: { error: { code: "SUBSCRIPTION_ACCESS_REQUIRED" } } });
  ...
  it("blocks missing and disabled feature entitlements with 403", async () => {
    ...
    await query("INSERT INTO workspace_entitlement_overrides (workspace_id, feature_key, enabled_override, reason) VALUES ($1, $2, false, 'test')", [workspaceId, featureKey]);
