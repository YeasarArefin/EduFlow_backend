...
describe("Better Auth email/password foundation", () => {
  ...
  afterAll(async () => {
    ...
    await pool.query("DELETE FROM workspace_entitlement_overrides WHERE workspace_id IN ($1, $2)", [
      workspaceId,
      deniedWorkspaceId
    ]);
  ...
  it("creates a user, creates a login session, and looks it up from its cookie", async () => {
    ...
    expect(contextResponse.body).toEqual({
      data: { userId: userResult.rows[0].id }
    });

    await pool.query("INSERT INTO workspaces (id, name, slug, status) VALUES ($1, $2, $3, 'active')", [
      workspaceId,
      "Auth Context Workspace",
      `auth-context-${suffix}`
    ]);
    await pool.query("INSERT INTO workspace_members (id, workspace_id, user_id, role_code) VALUES ($1, $2, $3, $4)", [
      membershipId,
      workspaceId,
    ...
    await pool.query(
      "INSERT INTO plans (id, name, slug, duration_days, trial_days) VALUES ($1, 'Pipeline Plan', $2, 30, 0)",
      [planId, `pipeline-${suffix}`]
    );
    await pool.query("INSERT INTO features (key, name) VALUES ('students', 'Students') ON CONFLICT (key) DO NOTHING");
    await pool.query(
      "INSERT INTO plan_features (plan_id, feature_key, enabled, limit_value) VALUES ($1, 'students', true, 100)",
      [planId]
    );
    await pool.query(
    ...
    expect(workspaceResponse.body).toEqual({
      data: { workspaceId, membershipId, roleCode: 101 }
    });

    await pool.query(
      "INSERT INTO role_permissions (role_code, permission_code) VALUES (101, 1101) ON CONFLICT DO NOTHING"
    );
    await request(app)
      .get("/api/v1/access-pipeline-example")
      .set("Cookie", cookie)
      .set("X-Workspace-Id", workspaceId)
      .expect(200);
    ...
    const blockedSubscriptionResponse = await request(app)
      .get("/api/v1/access-pipeline-example")
      .set("Cookie", cookie)
      .set("X-Workspace-Id", workspaceId)
      .expect(403);
    ...
    await pool.query(
      "INSERT INTO workspace_entitlement_overrides (workspace_id, feature_key, enabled_override, reason) VALUES ($1, 'students', false, 'pipeline test')",
      [workspaceId]
    );
    const disabledEntitlementResponse = await request(app)
      .get("/api/v1/access-pipeline-example")
      .set("Cookie", cookie)
      .set("X-Workspace-Id", workspaceId)
      .expect(403);
    ...
    await pool.query("INSERT INTO workspace_members (id, workspace_id, user_id, role_code) VALUES ($1, $2, $3, 201)", [
      deniedMembershipId,
      deniedWorkspaceId,
      userResult.rows[0].id
    ]);
    ...
    await pool.query(
      "INSERT INTO member_permission_overrides (workspace_id, member_id, permission_code, allowed) VALUES ($1, $2, 1101, true)",
    ...
    await pool.query(
      "UPDATE member_permission_overrides SET allowed = false WHERE workspace_id = $1 AND member_id = $2 AND permission_code = 1101",
      [workspaceId, membershipId]
    );
